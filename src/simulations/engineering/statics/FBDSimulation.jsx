...
export default function FBDSimulation() {
  ...
  useEffect(() => {
    ...
    const PRESETS = [
      { name: "Weight", label: "W", mag: 4, angle: 270, color: "#a9402f" },
      { name: "Normal", label: "N", mag: 3.5, angle: 90, color: "#4c7a3c" },
      { name: "Friction", label: "f", mag: 1.5, angle: 180, color: "#8a6500" },
      { name: "Applied", label: "F", mag: 2.5, angle: 0, color: "#2f5d88" },
      { name: "Tension", label: "T", mag: 3, angle: 45, color: "#24496b" },
      { name: "Drag", label: "D", mag: 1.2, angle: 180, color: "#5f564b" },
    ];
    ...
    function drawBody(c) {
      ...
      ctx.save();
      ctx.fillStyle = "#ead6a7";
      ctx.strokeStyle = "#2f5d88";
    ...
    function redraw() {
      ...
      for (const f of forces) {
        ...
        ctx.save();
        ctx.fillStyle = "#25231f"; ctx.font = "600 13px system-ui,sans-serif";
      ...
      if (showNet.checked && forces.length) {
        ...
        if (n.mag > 2) {
          ...
          drawArrow(c.x, c.y, tip.x, tip.y, "#a9402f", 2.5, [6, 6]);
          ctx.save();
          ctx.fillStyle = "#a9402f"; ctx.font = "600 12px system-ui,sans-serif";
    ...
    $("addForce").addEventListener("click", () => {
      addForce({ label: labelInput.value.trim() || "F", magnitude: parseFloat(magSlider.value) || 3, angleDeg: parseFloat(angSlider.value) || 0, color: "#2f5d88" });
    });
    ...
    addForce({ label: "N", magnitude: 3.5, angleDeg: 90, color: "#4c7a3c" });
  ...
  return (
    <div className="fbd-wrapper" ref={rootRef}>
      <header className="fbd-header">
        <div>
          <span className="fbd-eyebrow">Statics lab</span>
          <h1>Free Body Diagram</h1>
          <p>Build force vectors, inspect components, and run the net-force motion model from one workspace.</p>
        </div>
        <div className="fbd-equation" id="hint-mma"></div>
      </header>

      <main className="fbd-workspace">
        <section className="stage-wrap" aria-label="Free body diagram workspace">
          ...
          <p className="hint">Force magnitudes are in <strong>N</strong>; +y is up. Drag the block or an arrowhead to edit the diagram.</p>
        </section>

        <aside className="panel" aria-label="Free body diagram controls">
          ...
          <section className="control-card">
            ...
            <label>
              <span className="range-label">Magnitude <strong id="magVal">3</strong></span>
              <input type="range" id="magnitude" min="0.5" max="8" step="0.1" defaultValue="3" />
            </label>
            <label>
              <span className="range-label">Angle <strong><span id="angVal">270</span>°</strong></span>
              <small>0° right, 90° up</small>
              <input type="range" id="angle" min="0" max="359" step="1" defaultValue="270" />
            </label>
            <div className="button-grid two">
              <button type="button" className="btn btn-primary" id="addForce">Add force</button>
          ...
          <section className="control-card">
            ...
            <div className="toggles">
              ...
              <label><input type="checkbox" id="showComponents" /> Components</label>
          ...
          <section className="control-card">
            ...
            <div className="button-grid two">
              <button type="button" className="btn btn-primary" id="btnPlay">Play</button>
              <button type="button" className="btn btn-ghost" id="btnResetMotion">Reset</button>
            </div>
            <label>
              <span className="range-label">Mass <strong><span className="mono" id="massVal">2.0</span> kg</strong></span>
              <input type="range" id="massKg" min="0.5" max="20" step="0.1" defaultValue="2" />
            </label>
            <label>
              <span className="range-label">Time scale <strong><span className="mono" id="speedVal">1.0</span>×</strong></span>
              <input type="range" id="animSpeed" min="0.25" max="3" step="0.05" defaultValue="1" />
            </label>
            <label>
              <span className="range-label">Scale <strong><span className="mono" id="ppmVal">48</span> px/m</strong></span>
